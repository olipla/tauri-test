...
<template>
  <ClientOnly>
    ...
    >
      <div
        v-if="!serialDetails?.baudRate || !serialDetails?.port"
      ...
      >
        <div class="flex gap-2 shrink grow-0">
          <div class="flex flex-col gap-1 min-w-0">
            <div
              class="flex items-center justify-center h-full w-4 rounded-sm bg-accented text-xs relative overflow-hidden"
            >
              <div
                class="absolute inset-0 bg-primary opacity-0"
                :class="{ 'animate-[flash_0.2s_steps(1)_infinite]': props.receiving }"
              />
              <div class="z-10">
                R
              </div>
            ...
            >
              <div
                class="absolute inset-0 bg-primary opacity-0"
                :class="{ 'animate-[flash_0.2s_steps(1)_infinite]': props.transmitting }"
              />
              <div class="z-10">
                T
              </div>
        ...
        <div
          v-else
          class="flex flex-col grow items-center justify-center gap-1 w-full"
        >
          <UIcon
