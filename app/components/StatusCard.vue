  ...
  <CommonCard>
    <div class="flex gap-3 min-h-full">
      ...
      <div class="flex flex-col">
        ...
        <div v-if="issues.length > 1" class="flex gap-2 items-center">
          <div class="text-sm text-muted grow">
            Issue {{ issueIndex + 1 }} of {{ props.issues.length }}
          </div>
          <div class="flex">
            <UButton icon="i-lucide-chevron-left" variant="ghost" :ui="{ base: 'p-0' }" @click="decreaseIndex" />
            <UButton icon="i-lucide-chevron-right" variant="ghost" :ui="{ base: 'p-0' }" @click="increaseIndex" />
          </div>
        </div>
      </div>
    </div>
